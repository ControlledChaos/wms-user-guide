...
<div class="wrap">
	<h2><?php _e( 'Database Reset', 'wms-user-guide' ) ?></h2>
	<p class="description"><?php _e( 'Reset the WordPress database back to its original state.', 'wms-user-guide' ); ?></p>
	<hr />

	<?php include( 'partials/notice.php' ) ?>

	<form method="post" id="db-reset-form">
		<p>1. <?php _e( 'Select the database tables you would like to reset', 'wms-user-guide' ) ?>:</p>

		<div id="select-container">
			<a href='#' id="select-all"><?php _e( 'Select All', 'wms-user-guide' ) ?></a>
		...
		<p id="reactivate" style="display: none">
			...
			<?php _e( 'Reactivate current theme and plugins after reset?', 'wms-user-guide' ) ?>
			</label>
		</p>

		<p id="disclaimer" style="display: none">
			<em><?php printf( __( 'You selected the users table. Only the <strong><u>%s</u></strong> user will be restored', 'wms-user-guide' ), $this->user->user_login ) ?>.</em>
		</p>
		<p>
		2. <?php _e( 'Enter the security code into the text box', 'wms-user-guide' ) ?>:
		...
		<p>
			<input type="submit" name="db-reset-submit" value="<?php _e( 'Reset Tables', 'wms-user-guide' ) ?>" id="db-reset-submit" class="button-primary" disabled />
